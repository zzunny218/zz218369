...
export function classifyHandGesture(landmarks) {
  ...
  return {
    // 한 손가락이 잠깐 가려져도 편 손 제스처가 끊기지 않게 세 손가락 이상을 허용한다.
    isPalmOpen: extendedFingerCount >= 3,
...
export function getHandLandmarksBySide(result) {
  ...
  (result.landmarks ?? []).forEach((landmarks, index) => {
    ...
    const label = (categories[0]?.categoryName ?? "").toLowerCase();
    if (!landmarks) return;
    if ((label === "left" || label === "right") && !hands[label]) hands[label] = landmarks;
    else unassigned.push(landmarks);
  });

  // MediaPipe가 두 손을 같은 방향으로 순간 오분류해도 남은 손을 버리지 않는다.
  for (const landmarks of unassigned) {
    if (!hands.left) hands.left = landmarks;
    else if (!hands.right) hands.right = landmarks;
  }
